...
#define minWidth                                                               \
  78 // Defining the minimum width of the window in pixels - DON'T CHANGE THIS!!
#define BORDER(win)                                                            \
  wborder(win, 0, 0, 0, 0, 0, 0, 0,                                            \
          0) // Defining a macro for drawing a border around a border
...
// 2. write_to_string() - For using it with Curl and writing response to a
// string
...
// 7. getBucketDetails() - Getting the bucket details (in the form of string)
// and serializing it to a JSON structure through an API call
// 8. appendBucket() - Appending to the end of a bucket through an API call
...
bool corrupted() {
  if (localSave["number"] != localSave["data"].size() ||
...
void main_menu() {
  ...
  while (1) {
    ...
    if (getmaxx(stdscr) >= minWidth) {
      ...
      mvwprintw(stdscr, LINES / 2, (COLS - 35) / 2,
                "Please increase your window's width");
    }
    c = getch();
    switch (c) {
