...
std::optional<std::string> FileManager::readFile() const {
    ...
    spdlog::info("FileManager: Read file successfully: {} ({} bytes)", filename, content.size());
    return content;
}
...
bool FileManager::writeFile(const std::string& content) const {
    ...
    file << content;
    spdlog::info("FileManager: Wrote file successfully: {} ({} bytes)", filename, content.size());
    return true;
}

bool FileManager::deleteFile() const {
    if (!std::filesystem::exists(filename)) {
        spdlog::info("FileManager: Delete skipped, file does not exist: {}", filename);
        return true;
    }
    ...
    if (success) {
        ...
        spdlog::error("FileManager: Failed to delete file: {} - {}", filename, ec.message());
    }
    return success;
}
