...
bool Application::handleLogin() {
    ...
    while (true) {
        ...
        std::string hashed_pass = picosha2::bytes_to_hex_string(phash.begin(), phash.end());
        ...
        if (found) {
            ...
            if (matchedUser.password != hashed_pass) {
                spdlog::warn("Application: Authentication failed (wrong password) for user '{}'", uname);
                ui->loginPanel.showError("Wrong Password");
                ui->loginPanel.clearUsername();
                continue;
            }
            spdlog::info("Application: Authentication explicitly passed for user '{}'", uname);
        } else {
            ...
            picosha2::hash256(id_input.begin(), id_input.end(), ihash.begin(), ihash.end());
            
            matchedUser.id = picosha2::bytes_to_hex_string(ihash.begin(), ihash.end());
...
bool Application::mainLoop() {
    ...
    while (true) {
        ...
        int comp = 0;
        for (const auto& t: todos) if (t.isComplete) comp++;
        
        SyncStatus sync = syncManager->getSyncStatus();
        
        ui->mainMenuPanel.setCredentials(userManager.getCurrentUser()->name, currentPantryId);
