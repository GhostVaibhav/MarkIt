...
std::vector<Todo> TodoDBManager::getTodos(const User& user) {
    ...
    std::string sql = "SELECT id, name, \"desc\", time, isComplete FROM records_" + user.id;
    ...
    spdlog::info("TodoDBManager: Fetched {} todos for user {}", todos.size(), user.id);
    return todos;
}
...
bool TodoDBManager::addTodo(const User& user, const Todo& todo) {
    try {
        ...
        ensureTableExists(user);
        std::string sql = "INSERT INTO records_" + user.id + " (id, name, \"desc\", time, isComplete) VALUES (?, ?, ?, ?, ?)";
        SQLite::Statement query(db, sql);
        ...
        spdlog::error("TodoDBManager: Failed to add todo for user {}: {}", user.id, e.what());
        return false;
    }
}
...
bool TodoDBManager::removeTodo(const User& user, const Todo& todo) {
    try {
        ...
        query.bind(1, todo.id);
        query.exec();
        spdlog::info("TodoDBManager: Removed todo {} for user {}", todo.id, user.id);
        return true;
    } catch (const std::exception& e) {
        spdlog::error("TodoDBManager: Failed to remove todo for user {}: {}", user.id, e.what());
        return false;
    }
}
...
bool TodoDBManager::toggleTodo(const User& user, const Todo& todo) {
    try {
        if (user.id.empty()) return false;
        ensureTableExists(user);
        std::string sql = "UPDATE records_" + user.id + " SET isComplete = ? WHERE id = ?";
        SQLite::Statement query(db, sql);
        query.bind(1, todo.isComplete ? 1 : 0);
        query.bind(2, todo.id);
        query.exec();
        spdlog::info("TodoDBManager: Toggled todo {} for user {}", todo.id, user.id);
        return true;
    } catch (const std::exception& e) {
        spdlog::error("TodoDBManager: Failed to toggle todo for user {}: {}", user.id, e.what());
        return false;
    }
}
