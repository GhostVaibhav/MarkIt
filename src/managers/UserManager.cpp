...
UserManager::UserManager(const std::string& dbPath, const std::string& stateFilePath)
    : userDBManager(dbPath), stateFileManager(stateFilePath) {}

bool UserManager::addUser(const User& user) {
    if (userDBManager.existUser(user)) {
        spdlog::warn("UserManager: Attempted to add duplicate user '{}'", user.name);
        return false;
    }
...
bool UserManager::removeUser(const User& user) {
    ...
    if (dbRemoved && currentUser && currentUser->id == user.id) {
        spdlog::info("UserManager: Removed active user '{}', clearing session", user.name);
        clearSession();
    }
    return dbRemoved;
}
...
bool UserManager::saveSession() {
    ...
    spdlog::info("UserManager: Saved session for user '{}', result: {}", currentUser->name, result);
    return result;
}
...
bool UserManager::loadSession() {
    ...
    try {
        ...
        for (const auto& u : getAllUsers()) {
            if (u.id == sid) {
                spdlog::info("UserManager: Successfully loaded session for user '{}'", u.name);
                currentUser = u;
                return true;
            }
        }
