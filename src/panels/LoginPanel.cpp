...
void LoginPanel::render() {
    ...
    if (u_x < 0) u_x = 0;
    mvwprintw(userNameWindow, getmaxy(userNameWindow) / 2, u_x, "Username: %s", username.c_str());
    ...
    if (errorMessage == "Wrong Password") {
        ...
        if (err_x < 0) err_x = 0;
        mvwprintw(passwordWindow, getmaxy(passwordWindow) / 2, err_x, "Wrong Password");
        ...
        mvwprintw(passwordWindow, getmaxy(passwordWindow) / 2, pw_x, "%s", pwPrompt.c_str());
    }
