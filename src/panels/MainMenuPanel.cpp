...
MainMenuPanel::MainMenuPanel(WINDOW* w) 
    : FullScreenPanel(), loadingPanel(w), logoPanel(w, 0, 0), statsPanel(w, 0, 0) {}
...
void MainMenuPanel::setTodos(const std::vector<Todo>& todos) { todosList = todos; }
...
void MainMenuPanel::setCredentials(const std::string& username, const std::string& id) { curUser = username; pantryId = id; }
void MainMenuPanel::setStats(int total, int completed) { statsPanel.setStats(total, completed); }
void MainMenuPanel::setSyncStatus(int push, int pull) { statsPanel.setSyncStatus(push, pull); }
...
void MainMenuPanel::render() {
    ...
    if (!pantryId.empty() && pantryId != "None") {
        mvwprintw(todoUserName, 5, 3 * part + 25, "Pantry ID: %s", pantryId.c_str());
    }
    ...
    for (int i = 0; i < (int)todosList.size(); i++) {
        if (pointerIndex == i) {
            if (todosList.at(i).isComplete) wattron(todoBody, COLOR_PAIR(2));
            else wattron(todoBody, COLOR_PAIR(1));
        ...
        if (pointerIndex == i) {
            if (todosList[i].isComplete) wattroff(todoBody, COLOR_PAIR(2));
            else wattroff(todoBody, COLOR_PAIR(1));
