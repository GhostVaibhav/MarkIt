...
TodoDetailPanel::TodoDetailPanel()
    : FullScreenPanel(), logoPanel(win, 0, 0) {}
...
void TodoDetailPanel::render() {
    ...
    if (currentTodo.isComplete) wattron(win, COLOR_PAIR(2));
    else wattron(win, COLOR_PAIR(1));
    
    mvwprintw(win, yOffset, 5, "Status: %s", currentTodo.isComplete ? "Completed" : "Pending");
    
    if (currentTodo.isComplete) wattroff(win, COLOR_PAIR(2));
    else wattroff(win, COLOR_PAIR(1));
    ...
    wrefresh(win);
    refreshKeyBar({{"Up/Dn", "Move"}, {"Enter", "Choose"}, {"Esc/q/Q", "Back"}, {"^C", "Exit"}});
}
...
TodoDetailAction TodoDetailPanel::promptAction() {
    ...
    while (true) {
        ...
        else if (ch == KEY_DOWN && selected < (int)options.size() - 1) selected++;
        else if (ch == 3) return TodoDetailAction::QuitApp;
        else if (ch == 27 || ch == 'q' || ch == 'Q') return TodoDetailAction::Back;
        else if (ch == '\n') {
