...
std::string PantryFacade::getBaseUrl(const std::string& bucketName) const {
    return "https://getpantry.cloud/apiv1/pantry/" + pantryId + "/basket/" + bucketName;
}

bool PantryFacade::bucketExists(const std::string& bucketName) const {
    HttpRequestBuilder builder;
    builder.setUrl(getBaseUrl(bucketName)).setMethod("GET").addHeader("Content-Type: application/json");
...
bool PantryFacade::createBucket(const std::string& bucketName) const {
    HttpRequestBuilder builder;
    builder.setUrl(getBaseUrl(bucketName)).setMethod("POST")
...
bool PantryFacade::saveBucket(const std::string& bucketName, const nlohmann::json& data) const {
    HttpRequestBuilder builder;
    builder.setUrl(getBaseUrl(bucketName)).setMethod("PUT")
           .addHeader("Content-Type: application/json")
           .setBody(data.dump());
    auto response = builder.execute();
    return response.success && response.httpCode == 200;
