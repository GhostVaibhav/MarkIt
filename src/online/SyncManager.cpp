...
SyncResult SyncManager::pull(const std::string& userId, nlohmann::json& localData) {
...
SyncResult SyncManager::push(const std::string& userId, const nlohmann::json& localData) {
    spdlog::info("SyncManager: Initiating cloud data push for user '{}'", userId);
    nlohmann::json remoteData = pantryFacade.loadBucket(userId);
    if (remoteData.empty()) {
        spdlog::error("SyncManager: Cloud push validation failed (network or bucket empty)");
        return SyncResult::NetworkError;
    }
...
SyncResult SyncManager::pushData(const std::string& userId, const std::string& hash, const std::vector<Todo>& todos) {
...
SyncResult SyncManager::pullData(const std::string& userId, std::vector<Todo>& outTodos) {
    nlohmann::json localData;
    SyncResult result = pull(userId, localData);
    
    if (result == SyncResult::Success && localData.contains("data") && localData["data"].is_array()) {
        outTodos.clear();
...
SyncStatus SyncManager::refreshData(const std::string& userId, const std::string& hash, const std::vector<Todo>& todos) {
    nlohmann::json localData;
    localData["hash"] = hash;
    localData["number"] = 0;
